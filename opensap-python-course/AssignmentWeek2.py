...
Therefore, nothing needs to be produced and the resulting stock is 200 (= 500 - 300). In the second month 250 pieces are sold. 
The stock level after the previous month is 200. Therefore 50 pieces need to be produced. The resulting stock level is 0 (= 200 + 50 - 250). 
...
number_of_months = int(input("Please enter the number of month to plan: "))
planned_sales_quantity = []

for i in range(number_of_months):
    planned_sales_quantity.append(int(input("Please enter the planned sales quantity: ")))

required_production_quantity = 0
diff = 0

print("The resulting production quantities are:")
for k in range(number_of_months):
    diff = stock_level - planned_sales_quantity[k]
    
    if diff > 0:
        required_production_quantity = 0        
    else:
        required_production_quantity = diff * -1
    
    print("Production quantity month", k + 1, "-", required_production_quantity)
    stock_level -= (planned_sales_quantity[k] - required_production_quantity)